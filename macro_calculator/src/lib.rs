...
pub fn calculate_macros(foods: Vec<Food>) -> json::JsonValue {
    let res = foods.into_iter().fold(object! {
        ...
        object! {
           "cals" : acc["cals"].as_f64().unwrap() + sum_cals(food.calories.to_vec()) * food.nbr_of_portions,
...
mod tests {
    ...
    fn it_works() {
        ...
        assert_eq!(res["cals"].as_f32().unwrap(), 2777.39);

        assert_eq!(res["carbs"].as_f32().unwrap(), 322.44);
        assert_eq!(res["proteins"].as_f32().unwrap(), 122.06);
        assert_eq!(res["fats"].as_f32().unwrap(), 106.93);
    }
}
