...
// rm_all_ref: which accepts an Rc<String> and removes all elements from the vector that are equal to that value.
//      This should only happen if the two Rcs point to the same allocation.
//
...
impl Node {
    ...
    pub fn rm_all_ref(&mut self, element: Rc<String>) {
        self.ref_list.retain(|r| r != &element);
    }
}
